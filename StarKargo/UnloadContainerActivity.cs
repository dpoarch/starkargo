...
namespace StarKargo
{
    ...
    public class UnloadContainerActivity : Activity
    {
        ...
        Guid SELECTED_SHIIPEDLOTNO = Guid.Empty;
        string SELECTED_SHIPPEDLOTNOSTR = String.Empty;

        protected override void OnCreate(Bundle bundle)
        {
            base.OnCreate(bundle);

            SetContentView(Resource.Layout.UnloadContainer);

            InitializeListView();

            // Shipped Lot ID
            Spinner spinnnerLot = FindViewById<Spinner>(Resource.Id.spinShippedLotID);
            var adapterLot = new ArrayAdapter(this, Resource.Layout.spinner_item, UserSession.ShippedLotsStringList);
            spinnnerLot.ItemSelected += new EventHandler<AdapterView.ItemSelectedEventArgs>(spinnerShippedLot_ItemSelected);
            ...
            completeAction.Click += (object sender, EventArgs e) =>
            {
                ...
                if (String.IsNullOrEmpty(SELECTED_SHIPPEDLOTNOSTR))
                {
        ...
        private void AddNewPackage()
        {
            ...
            if (GetConnectionStatus())
            {
                ...
                if (model.Status != (int)StatusTypeEnums.SHIPPED)
                {
        ...
        private void ProcessUnLoadContainer()
        {
            foreach (var item in tableItems)
            {
                ...
                if (GetConnectionStatus())
                {
                    try
                    {
                        ...
                        if (tempModel.Status != (int)StatusTypeEnums.SHIPPED)
                        {
                            continue;
                        }
                    }
                ...
                {
                    LotNo = SELECTED_SHIIPEDLOTNO,
                    LotNoStr = SELECTED_SHIPPEDLOTNOSTR,
                    OrderNumber = item.Heading,
                    PackageQty = item.PackageQty,
                    UnloadedTS = DateTime.UtcNow,
                    Status = (int)StatusTypeEnums.UNLOADED,
                    GUID = item.GUID
                };
        ...
        private void AddToLocalLoadContainerTable(LoadContainerModel model)
        {
            try
            {
                ...
                // insert new item
                UnLoadContainerTable tbl = new UnLoadContainerTable
                {
                    ...
                    UnloadedTS = model.ModelOrder.LoadedTS
                };
        ...
        private void spinnerShippedLot_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
        {
            Spinner spinner = (Spinner)sender;

            var lotStr = Convert.ToString(spinner.GetItemAtPosition(e.Position));
            var lotModel = UserSession.ShippedLots.Where(x => x.Value == lotStr).FirstOrDefault();
            if (lotModel != null)
            {
                SELECTED_SHIIPEDLOTNO = lotModel.GUID;
                SELECTED_SHIPPEDLOTNOSTR = lotModel.Value;
            }
        }
        ...
        void RemoveItemFromList()
        {
            if (!String.IsNullOrEmpty(selectedBarcode))
            {
                ...
                if (item != null)
                {
                    ...
                    // totalCount = tableItems.Sum(x => x.PackageCount);
                    //   scanPackageCountTxt.Text = string.Format("{0} Packages Scanned", totalCount);
                }
            }
        }
